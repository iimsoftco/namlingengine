...
#include <random>
#include <algorithm>
#include <set>  // for touched boxes tracking
...
int main() {
    sf::RenderWindow window(sf::VideoMode(800, 600), "Namling Engine v2.1");
    window.setFramerateLimit(60);

    sf::Texture namlingTexture;
    if (!namlingTexture.loadFromFile("assets/namling.png")) {
        std::cerr << "Failed to load assets/namling.png\n";
        return 1;
    }
    ...
    std::set<size_t> touchedBoxes; // To track boxes touched by player
    ...
    while (window.isOpen()) {
        ...
        // Always allow WASD movement in addition to script
        ...
        // Track which boxes have been touched
        for (size_t i = 0; i < ctx.boxes.size(); ++i) {
            if (ctx.boxes[i].getGlobalBounds().intersects(namling.getGlobalBounds())) {
                touchedBoxes.insert(i);
            }
        }

        // Remove boxes that have been touched persistently
        std::vector<sf::RectangleShape> newBoxes;
        for (size_t i = 0; i < ctx.boxes.size(); ++i) {
            if (touchedBoxes.find(i) == touchedBoxes.end()) {
                newBoxes.push_back(ctx.boxes[i]);
            }
        }
        ctx.boxes = std::move(newBoxes);

        // Update box count in Namlinx
        ctx.vars["box_count"] = static_cast<float>(ctx.boxes.size());

        // Win condition: when no boxes remain
        if (ctx.boxes.empty()) {
            ctx.vars["win"] = 1;
        } else {
            ctx.vars["win"] = 0;
        ...
        // Display win text if win condition is met
        if (ctx.vars.count("win") && ctx.vars["win"] == 1) {
            ctx.str_vars["text"] = "YOU WIN";
            ctx.vars["text_x"] = 300;
            ctx.vars["text_y"] = 20;
        } else {
            ctx.str_vars.erase("text");
        }
